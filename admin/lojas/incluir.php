<?
	...
	{
		$btLabel 		= "Incluir";
		$head 			= "I n c l u i r&nbsp;&nbsp;&nbsp;&nbsp;L o j a";
	}
	elseif( $_GET['action'] == "alterar" )
	{
		$btLabel 		= "Alterar";
		$head 			= "A l t e r a r&nbsp;&nbsp;&nbsp;&nbsp;L o j a";
		$linhaReg		= $loja->getOneLoja($_REQUEST['idLoja']);
	}
	...
	if( $_POST['btEnviar'] == $btLabel )
	{
		...
		elseif( $_GET['action'] == "alterar" )
		{
			if( $loja->alterLoja() )
				...
				header("Location: incluir.php?action=alterar&idLoja=".$_POST['idLoja']);
			die();
		}
	}
	...
	$comboCategorias = "<select name='idCategoria' class='combo'>";
	for( $i=0; $i<count($arrCategorias); $i++ )
	{
		if( $linhaReg->idCategoria == $arrCategorias[$i]->idCategoria )
			$selected = "selected";
		else
			$selected = "";
	...
	<tr>
		<td style="padding-top: 10px;">
			<form name="form" id="form" action="incluir.php?action=<?=($_GET['action'] ? $_GET['action'] : "incluir")?>" method="POST" onsubmit="return submitForm();">
				<input type='hidden' name='idLoja' value='<?=$_GET['idLoja']?>' />
				<table cellpadding="5" cellspacing="0" border="0" align="center">
                    ...
                    <tr>
                        <td style="width: 30%;" id="txtNome">Nome da Loja:</td>
