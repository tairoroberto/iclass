$(document).ready(function(){
	...
	$("#linkRegulamento").click(function(){ 
		$(".mdl-termos").show();
		$("html").addClass("modal-over");
	});
	// bg
	$(".bg-modal").click(function(){ 
		$(this).parent().hide();
		$("html").removeClass("modal-over");
	});
	// fechar
	...
	$(".btn-fechar-light").click(function(){ 
		$(this).parent().parent().hide();
	});
})

// DADOS DOS TREINAMENTOS E DO QUIZ
var slides = new Array();
var slideAtual = 1;
var idTreinamentoAtual = "";
var idQuizPerguntaAtual = "";
var intervalQuestao = "";
var tempoAtual = 0;

function abreQuiz()
{
	$(".divFrameTreinamento").hide("fast");
	$(".divFrameTreinamento").html("");
	$(".divFrameQuiz").show("fast");
	$(".divLoadingQuiz").show();
	$(".navegacaoTreinamento").hide();
	clearInterval(intervalQuestao);
	intervalQuestao = "";
	
	//Pega a primeira questão
	$.post('pages/primeiraQuestao.php', { idTreinamento: idTreinamentoAtual }, function(data) {
		//pega os dados da primeira questão
		$.post('pages/dadosQuiz.php', { }, function(data2) {
			var dadosQuestao = data2.split("|");
			$(".navegacaoQuiz").show();
			$(".divFrameQuiz").html(data);
			$("#divNumeroPerguntas").html(dadosQuestao[0] + " de " + dadosQuestao[2]);
			$("#tempoQuestao").html("Tempo: " + dadosQuestao[1]);
			tempoAtual = parseInt(dadosQuestao[1]);
			idQuizPerguntaAtual = dadosQuestao[3];
			intervalQuestao = setInterval(computaTempoQuestao, 1000);
		});
	});
}

function computaTempoQuestao()
{
	if( tempoAtual > 0 )
	{
		tempoAtual--;
		$("#tempoQuestao").html("Tempo: " + tempoAtual);
	}
	else //encerrou o tempo, passa pra próxima questão
	{
		$(".divFrameQuiz").html('<div class="divLoadingQuiz">Carregando...<br /><img src="Util/img/loader_quiz.gif" border="0" /></div>'); //volta a tela de loading
		clearInterval(intervalQuestao);
		intervalQuestao = "";
		proximaQuestao();
	}
...
{
	var idQuizRespostaAtual = $('input[name="radioResposta"]:checked').val();
	$(".divFrameQuiz").html('<div class="divLoadingQuiz">Carregando...<br /><img src="Util/img/loader_quiz.gif" border="0" /></div>'); //volta a tela de loading
	clearInterval(intervalQuestao);
	intervalQuestao = "";
	//Pega a questão seguinte
	$.post('pages/proximaQuestao.php', { idTreinamento: idTreinamentoAtual, idQuizPergunta: idQuizPerguntaAtual, idQuizResposta : idQuizRespostaAtual }, function(data) {
		//pega os dados da primeira questão
		$.post('pages/dadosQuiz.php', { }, function(data2) {
			if( data2 != "0" )
			{
...
function fechar() {
	document.getElementById('modal').style.filter = "alpha('opacity=0')";
	document.getElementById('modal').style.opacity = "0";
	document.getElementById('modal').style.top = "-225px";
	document.getElementById('modal').style.left = "-375px";
